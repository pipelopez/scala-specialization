...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet =
    ...
    if p(elem) then filteredRight.incl(elem) else filteredRight
  ...
  def descendingByRetweet: TweetList =
    if left.isInstanceOf[Empty] && right.isInstanceOf[Empty] then Cons(elem, Nil)
    else
  ...
  def mostRetweeted: Tweet =
    ...
    else if elem.retweets > rightMost.retweets then elem else rightMost
