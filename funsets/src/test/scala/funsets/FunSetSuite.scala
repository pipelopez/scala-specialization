...
class FunSetSuite extends munit.FunSuite:
  ...
  test("diff contains elements of the first set not in the second set") {
    new TestSets:
  ...
  test("filter returns subset of elements that satisfy the predicate") {
    new TestSets:
      val s = filter(union(s1, union(s2, s3)), x => x % 2 == 1)
      assert(contains(s, 1), "Filter 1")
      assert(!contains(s, 2), "Filter 2")
      assert(contains(s, 3), "Filter 3")
  }

  test("forall returns true if all elements in the set satisfy the predicate") {
    new TestSets:
  ...
  test("exists returns true if any element in the set satisfies the predicate") {
    new TestSets:
      val s = union(s1, union(s2, s3))
