...
trait Huffman extends HuffmanInterface:
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case t1 :: t2 :: ts => (makeCodeTree(t1, t2) :: ts).sortBy(weight)
    case _ => trees
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] =
    def decodeAcc(t: CodeTree, bs: List[Bit], acc: List[Char]): List[Char] = t match {
      case Leaf(c, _) if bs.isEmpty => acc :+ c
      case Leaf(c, _) => decodeAcc(tree, bs, acc :+ c)
      case Fork(left, right, _, _) =>
        if (bs.head == 0) decodeAcc(left, bs.tail, acc)
        else decodeAcc(right, bs.tail, acc)
    }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] =
    def encodeChar(t: CodeTree, c: Char): List[Bit] = t match {
      case Leaf(char, _) if char == c => List()
      case Fork(left, right, _, _) =>
  ...
  def convert(tree: CodeTree): CodeTable =
    def convertAcc(t: CodeTree, prefix: List[Bit]): CodeTable = t match {
      case Leaf(char, _) => List((char, prefix))
      case Fork(left, right, _, _) =>
        mergeCodeTables(convertAcc(left, prefix :+ 0), convertAcc(right, prefix :+ 1))
    }
