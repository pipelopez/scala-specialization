...
object Anagrams extends AnagramsInterface:
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    def combine(chars: List[(Char, Int)]): List[List[(Char, Int)]] = chars match {
      case Nil => List(List())
      case (char, count) :: tail =>
        val tailCombinations = combine(tail)
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    val yMap = y.toMap
    x.map { case (char, count) => (char, count - yMap.getOrElse(char, 0)) }.filter(_._2 > 0)
